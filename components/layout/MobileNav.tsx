...
import { MEGA_MENU_ITEMS } from '@/lib/megaMenuData';
...
export default function MobileNav({ open, onClose }: MobileNavProps) {
  ...
  return createPortal(
    ...
    >
      ...
      <nav id="mobile-navigation" ref={drawerRef} className="drawer-in absolute inset-y-0 right-0 flex h-dvh w-full max-w-[30rem] flex-col border-l border-ink-950/10 bg-paper-50 shadow-2xl">
        <div className="shrink-0 border-b border-ink-950/12 bg-white px-4 py-3 sm:px-5">
          ...
          <div className="mt-3 grid grid-cols-2 sm:grid-cols-4 overflow-hidden rounded-[8px] border border-ink-950/12 bg-ink-950/8">
            {QUICK_LINKS.map((item) => (
              <Link
                ...
                className={`safe-focus flex min-h-[44px] items-center justify-center border-r border-b sm:border-b-0 border-ink-950/10 px-2 text-center text-[0.64rem] font-black uppercase tracking-[0.06em] last:border-r-0 ${
                  isActive(item.href)
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-4 sm:px-5">
          <div className="mb-4 flex items-end justify-between gap-4">
            <div>
              <p id="mobile-navigation-title" className="text-[0.66rem] font-black uppercase tracking-[0.16em] text-frad-green-800">Menu</p>
              <p className="mt-1 text-sm font-semibold text-ink-600">Explore our programmes, locations, results, and ways to connect.</p>
            </div>
            <Link
              href="/contact"
              onClick={onClose}
          ...
          <div className="space-y-2.5">
            {MEGA_MENU_ITEMS.map((item) => {
              const activeGroup = isActive(item.href) || item.sections.some((section) =>
                isActive(section.href) || section.items.some((child) => isActive(child.href))
              );
              ...
              return (
                ...
                >
                  <div className="flex min-h-[42px] items-center rounded-[8px] px-2 text-sm font-black uppercase tracking-[0.08em] text-ink-950">
                    <span>{item.label}</span>
                  </div>

                  <div className="mt-2 space-y-3 border-t border-ink-950/8 pt-3">
                    {item.sections.map((section) => (
                      <div key={`${item.id}-${section.title}`} className="px-2">
                        <Link
                          href={section.href}
                          onClick={onClose}
                          className="safe-focus flex items-center justify-between py-1 text-xs font-black uppercase tracking-[0.1em] text-frad-green-900"
                        >
                          {section.title}
                          <span aria-hidden="true">→</span>
                        </Link>
                        <div className="mt-1.5 grid gap-1 border-l border-frad-green-800/25 pl-2">
                          {section.items.map((child) => (
                            <Link
                              key={`${section.title}-${child.href}`}
                              href={child.href}
                              onClick={onClose}
                              aria-current={isActive(child.href) ? 'page' : undefined}
                              className={`safe-focus rounded-lg px-3 py-2 text-sm font-semibold leading-snug ${
                                isActive(child.href)
