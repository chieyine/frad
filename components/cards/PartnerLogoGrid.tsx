  ...
  return (
    <div className="grid grid-cols-2 items-center gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
      {partners.map((partner) => (
        <div
          key={partner.name}
          className="group flex h-24 items-center justify-center rounded-[8px] border border-ink-950/10 bg-white/90 p-4 transition-all duration-300 hover:-translate-y-1 hover:border-frad-green-800/30 hover:shadow-[0_18px_42px_-28px_rgba(8,17,13,0.32)]"
          title={partner.name}
        >
          {partner.logo ? (
            <Image
